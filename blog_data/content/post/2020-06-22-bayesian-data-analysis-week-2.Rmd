...
labels_beta <- unlist(map(seq(10, 250, length.out = 4), ~ glue::glue("beta = {.}")))
labels_prior_observations <- unlist(map(seq(2, 50, length.out = 4) + seq(10, 250, length.out = 4) - 2,
                                    ~ glue::glue("prior obs = {.}")))

tibble(alpha = seq(2, 50, length.out = 4),
         beta = seq(10, 250, length.out = 4)) %>% 
  ...
  labs(title = "Prior Sensitivity Analysis",
       subtitle = "Original posterior mean shown in red. Subsequent priors are increasingly concentrated around 0.16")

```

The posterior is very robust. Even after drastically increasing the number of prior observations, the posterior distribution hardly changes. 
